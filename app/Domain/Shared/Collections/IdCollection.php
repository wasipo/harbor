...
/**
 * Base class for ID collections
 * 
 * @template TId of Stringable
 * 
 ...
 * PHP's type system limitations than pretending 'Stringable' provides safety.
 */
abstract class IdCollection implements Countable
{
     ...
     * @return static
     */
    public static function of(Stringable ...$ids): static
    {
        /** @var list<TId> $ids */
    ...
    {
        return $this->ids->contains(fn (mixed $item) => (string) $item === (string) $id);
    }
    ...
    {
        /** @var list<string> */
        return $this->ids->map(fn ($id) => (string) $id)->values()->all();
    }
